...
STAR_STATUS star_platform_init(void) {
  ...
  pfnNtQuerySystemInformation =
      (PFN_NtQuerySystemInformation)(void *)GetProcAddress(
          g_ntdll, "NtQuerySystemInformation");
  pfnNtQueryVirtualMemory = (PFN_NtQueryVirtualMemory)(void *)GetProcAddress(
      g_ntdll, "NtQueryVirtualMemory");
...
STAR_STATUS star_platform_enum_processes(STAR_PROCESS_LIST *list) {
  ...
  do {
    ...
    status = pfnNtQuerySystemInformation(SystemProcessInformation_Star, buffer,
  ...
  /* BYTE *current = (BYTE *)buffer; - unused */
  while (1) {
  ...
  do {
    ...
    if (hProcess) {
      ...
      if (GetProcessTimes(hProcess, &create_time, &exit_time, &kernel_time,
                          &user_time)) {
        ULARGE_INTEGER ul;
  ...
  *regions = NULL;
  ...
  while (VirtualQueryEx(hProcess, addr, &mbi, sizeof(mbi))) {
    if (mbi.State == MEM_COMMIT) {
      STAR_MEMORY_REGION *region =
          (STAR_MEMORY_REGION *)calloc(1, sizeof(STAR_MEMORY_REGION));
      if (!region) {
        CloseHandle(hProcess);
      ...
      region->is_executable = (mbi.Protect & (PAGE_EXECUTE | PAGE_EXECUTE_READ |
                                              PAGE_EXECUTE_READWRITE |
                                              PAGE_EXECUTE_WRITECOPY)) != 0;
      region->is_writable = (mbi.Protect & (PAGE_READWRITE | PAGE_WRITECOPY |
                                            PAGE_EXECUTE_READWRITE |
                                            PAGE_EXECUTE_WRITECOPY)) != 0;
      ...
      if (region->is_executable && mbi.Type != MEM_IMAGE) {
        BYTE pe_check[2] = {0};
        SIZE_T bytes_read = 0;
        if (ReadProcessMemory(hProcess, mbi.BaseAddress, pe_check, 2,
                              &bytes_read)) {
      ...
      if (mbi.Type == MEM_IMAGE) {
        GetMappedFileNameA(hProcess, mbi.BaseAddress, region->mapped_file,
                           STAR_MAX_PATH);
      }
    ...
    if ((BYTE *)mbi.BaseAddress + mbi.RegionSize < addr)
      break; /* Overflow */
  }
...
STAR_STATUS star_platform_read_process_memory(uint32_t pid, uint64_t address,
  ...
  *bytes_read = 0;
  ...
  SIZE_T read = 0;
  BOOL ok = ReadProcessMemory(hProcess, (LPCVOID)(uintptr_t)address, buffer,
                              size, &read);
  CloseHandle(hProcess);
